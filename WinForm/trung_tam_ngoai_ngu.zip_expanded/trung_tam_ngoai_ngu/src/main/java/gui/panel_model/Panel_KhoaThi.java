...
import gui.frame_model.Frame_Popup_KhoaThi;

public class Panel_KhoaThi extends JPanel {
	/**
	 * 
	 */
	...
	BUS_KhoaThi bus_KhoaThi;
	String[] columns = {"Tên khóa thi", "Ngày thi"};
	String[][] tableData;
	DTO_KhoaThi[] arrKhoaThi;
	
	public Panel_KhoaThi() {
		bus_KhoaThi = new BUS_KhoaThi();
		...
		field1 = new Panel_Field("Tên khóa thi", jTextField1);
		...
		field2 = new Panel_Field("Ngày thi", jDateChooser2);
		...
		btnCreate.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				JFrame add = new Frame_Popup_KhoaThi("Thêm khóa thi", true, null);
				add.setVisible(true);
		...
		btnEdit.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(table_1.getSelectedRow() >= 0) {
					khoaThi = arrKhoaThi[table_1.getSelectedRow()];
					JFrame edit = new Frame_Popup_KhoaThi("Chỉnh sửa khóa thi", true, khoaThi);
		...
		btnView.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(table_1.getSelectedRow() >= 0) {
					khoaThi = arrKhoaThi[table_1.getSelectedRow()];
					JFrame view = new Frame_Popup_KhoaThi("Xem thông tin khóa thi", false, khoaThi);
	...
	public void updateTable() {
		...
		dto_KhoaThi.settenKhoaThi(jTextField1.getText());
		dto_KhoaThi.setngayThi(support.Method.getDateFromJDate(jDateChooser2.getDate()));
		
		DTO_KhoaThi[] temp = bus_KhoaThi.getData(dto_KhoaThi);
		arrKhoaThi = new DTO_KhoaThi[temp.length];
		System.arraycopy(temp, 0, arrKhoaThi, 0, temp.length);;
		tableData = new String[arrKhoaThi.length][2];;
		for(int i = 0; i < arrKhoaThi.length; i++) {
			for(int j = 0; j < tableData[i].length; j++) {
				tableData[i][j] = arrKhoaThi[i].getTableData()[j];
			}
		}
