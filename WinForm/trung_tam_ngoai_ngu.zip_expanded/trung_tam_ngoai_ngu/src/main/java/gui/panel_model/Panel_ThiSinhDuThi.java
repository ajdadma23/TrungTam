...
import bus.BUS_ThiSinhDuThi;
import dto.DTO_ThiSinhDuThi;
import gui.Panel_Field;
import gui.frame_model.Frame_Popup_ThiSinhDuThi;

public class Panel_ThiSinhDuThi extends JPanel {
	/**
	 * 
	 */
	...
	BUS_ThiSinhDuThi bus_ThiSinhDuThi;
	String[] columns = {"Số báo danh", "Tên thí sinh", "CMND/CCCD", "Số điện thoại", "Phòng thi"};
	String[][] tableData;
	int numRow = 5;
	DTO_ThiSinhDuThi[] arrThiSinhDuThi;
	
	public Panel_ThiSinhDuThi() {
		bus_ThiSinhDuThi = new BUS_ThiSinhDuThi();
		...
		JPanel field1 = new Panel_Field("Tên thí sinh", jTextField1);
		GridBagLayout gridBagLayout_2 = (GridBagLayout) field1.getLayout();
		gridBagLayout_2.rowHeights = new int[] {80, 0};
		search.add(field1);
		
		JPanel field2 = new Panel_Field("Số báo danh", jTextField2);
		...
		btnCreate.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				JFrame add = new Frame_Popup_ThiSinhDuThi("Thêm khóa thi", true, null);
				add.setVisible(true);
		...
		btnDelete.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				DTO_ThiSinhDuThi thiSinh;
				if(table_1.getSelectedRow() >= 0) {
					thiSinh = arrThiSinhDuThi[table_1.getSelectedRow()];
					bus_ThiSinhDuThi.delete(thiSinh);
					updateTable();
				}else {
	...
	public void updateTable() {
		DTO_ThiSinhDuThi dto_ThiSinhDuThi = new DTO_ThiSinhDuThi();
		dto_ThiSinhDuThi.setHoTen(jTextField1.getText());
		dto_ThiSinhDuThi.setsbd(jTextField2.getText());
		dto_ThiSinhDuThi.setcmnd(jTextField3.getText());
		dto_ThiSinhDuThi.setSdt(jTextField4.getText());
		
		DTO_ThiSinhDuThi[] temp = bus_ThiSinhDuThi.getData(dto_ThiSinhDuThi);
		arrThiSinhDuThi = new DTO_ThiSinhDuThi[temp.length];
		System.arraycopy(temp, 0, arrThiSinhDuThi, 0, temp.length);;
		tableData = new String[arrThiSinhDuThi.length][numRow];;
		for(int i = 0; i < arrThiSinhDuThi.length; i++) {
			for(int j = 0; j < tableData[i].length; j++) {
				tableData[i][j] = arrThiSinhDuThi[i].getTableData()[j];
			}
		}
